...
- Use `read_only` for explanation, planning, state checks, and read-only review; use `design` while collecting policy; use `write` for approved managed changes; use `governance_high_risk` for release or merge review. Only the last two may request a review subagent.
...
The global baseline owns only cross-repository defaults: instruction scope, managed entry, reuse-first execution, advisory task rating, the four-part `Coding Behavior Baseline`, `Done When`, dual Python/script preflight, comments/documentation, environment/dependency safety, installed-skill protection, and Markdown `$...$` / `$$...$$` formulas. Project thresholds, layouts, exceptions, long-task rules, language detail, and releases stay in project governance.
...
- Treat installed skill directories as read-only unless the user explicitly authorizes installation, replacement, or direct modification.
