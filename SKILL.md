...
6. **Docs Governance**
   ...
   - 如果 `.agents/docs-governance-state.json` 中的 `last_evolution_target` 仍是旧的字符串路径或其他坏格式，不要沿用坏状态继续推断。使用 `python skills/agents-md-generator/scripts/manage_docs.py evolve <project> --force` 或重新应用新的 experience payload 进行修复；修复后的状态必须把 `last_evolution_target` 写回规范对象，而不是文件路径字符串。
...
7. **Verify**
   ...
   - Use `python skills/agents-md-generator/scripts/run_skill_evals.py skills/agents-md-generator/evals/evals.json` for the repo-local skill-effectiveness layer. These evals must compare with-skill and without-skill expectations, report whether the skill actually improves behavior, cover all five Skill design patterns, and cover recent high-risk regressions such as missing-root detection, takeover routing, root-level whitelist gating, exact-cwd evolution review, generic audit/evaluate behavior, release completeness blocking, configurable code comment policy enforcement, and packaged eval runtime independence without repository-only tests.
   ...
   - Use `python skills/agents-md-generator/scripts/run_confidence_gate.py <project> --review-base <sha> --external-skill-dir <healthy-skill-dir>` when you need one aggregate evidence pass that includes quick validation, docs governance, work-folder governance, freshness, automated review governance, verify, evaluate, skill-effectiveness evals, isolated release-package evals when a matching dist release exists, release gates, install-skip, and a healthy real external skill smoke.
