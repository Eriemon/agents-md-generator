...
7. **Verify**
   ...
   - Use `python skills/agents-md-generator/scripts/run_skill_evals.py skills/agents-md-generator/evals/evals.json` for the repo-local skill-effectiveness layer. These evals must compare with-skill and without-skill expectations, report whether the skill actually improves behavior, cover all five Skill design patterns, and cover recent high-risk regressions such as missing-root detection, takeover routing, root-level whitelist gating, exact-cwd evolution review, generic audit/evaluate behavior, release completeness blocking, and packaged eval runtime independence without repository-only tests.
