...
- Keep Code Comment Policy handling aligned across rendered AGENTS.md output, verification rules, reference review docs, eval coverage, and `agents/openai.yaml`; generated roots must summarize the configurable `code_comment_policy` from `.agents/global-rule-overrides.json` and keep Python/C/C++/Verilog rules out of long root examples.
...
- A `## Code Comment Policy` section that points to `code_comment_policy` in `.agents/global-rule-overrides.json`, summarizes the Chinese Python/C/C++/Verilog policy, forbids obvious restatement comments, requires stale comments to be updated, blocks bulk AI-generated commenting unless explicitly requested, and keeps detailed examples in `references/code-comment-policy.md`.
...
- Require the rendered root `AGENTS.md` to keep the Code Comment Policy; verification must reject managed roots that remove the section, lose `code_comment_policy`, or weaken any Python/C/C++/Verilog language rule, stale-comment update rule, formatting separation rule, or bulk-AI-commenting prohibition.
...
- The generated root includes `## Code Comment Policy`, the policy points to `code_comment_policy`, summarizes Python/C/C++/Verilog rules, and verification fails if the policy is missing or weakened.
