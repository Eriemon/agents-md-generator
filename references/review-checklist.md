...
- [ ] `coding_behavior.language_skill_routing` has exactly `shared`, `python`, and `script`; `shared` is 只渲染一次.
...
- [ ] The root retains readable formatting, `严禁把代码压缩到一行`, and the obfuscated-code prohibition.
