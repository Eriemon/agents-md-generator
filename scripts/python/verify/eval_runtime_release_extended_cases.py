...
def governance_cli_checks(
    ...
    list_planning_terms = [  # 工作目录规划触发词
        ...
        "规划",  # 中文规划触发词
        "准备",  # 中文准备触发词
        "current workspace/current repository/current work folder",  # 英文工作目录范围词
    ]
    ...
    return {
        ...
        "remote_contract_mentions_workspace_check": "workspace-check" in str_agents
        or "workspace-check" in str_skill,
        "remote_review_blocks_local_settings": any(
            "local-only workspace settings" in str_reason
            for str_reason in dict_remote_review.get("reasons", [])
        ),
