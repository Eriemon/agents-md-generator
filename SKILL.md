...
2. **Design Interview**
   ...
   - A grouped interview is not complete until the final `alignment_confirmed` confirmation succeeds. Takeover mode may still auto-synthesize non-directory descriptive fields after the minimum identity answers, but it must not auto-confirm or auto-fill the directory contract.
