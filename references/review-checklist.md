...
| Skill effectiveness | `python skills/agents-md-generator/scripts/run_skill_evals.py skills/agents-md-generator/evals/evals.json` passes, every case reports with-skill improvement over the without-skill baseline, recent high-risk regressions and all five Skill design patterns are covered, and the eval runner does not depend on repository-only `tests/` helpers |
...
- `manage_docs.py verify` should catch current-version drift across source `VERSION`, root AGENTS metadata/control profile, DEVELOPMENT, CHANGELOG, and GIT_MANAGER when those version fields are present.
...
- Every tenth handoff should evolve accepted experience into indexed templates under `assets/templates/evolution/`, but only from AI-authored `evolution_summary` synthesis, and the tenth-handoff apply should fail atomically when the summary is missing or invalid.
- Every tenth handoff also requires an extra evolution review gate: `requires_extra_evolution_review=true`, `review_scope=evolution-only`, `review_blocking=true`, and a populated `review_contract` plus `release_alignment_evidence` must appear in `.agents/experience-update-request.json`.
...
- Tenth-handoff payloads must include `evolution_review` with `verdict`, `approved_target`, `original_target`, `evidence_read`, `session_reread_performed`, `session_reread_reason`, and `full_explanation`; missing review state must block evolution and write `.agents/evolution-review-request.json`.
