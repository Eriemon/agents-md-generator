...
def confirm_group(project: Path, state: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any]:
    ...
    correction_keys = [key for key in payload if key != GROUP_CONFIRMATION_KEY]  # 本次提交的修正字段。

    # 当前组问题标识限定修正范围。
    list_group_ids = current_group_ids(state)  # 当前交互组的问题标识列表。

    # 答案字段名用于直接校验 payload 键。
    expected_keys = question_ids_to_keys(list_group_ids)  # 当前组允许修改的答案字段。
    ...
    # 否认确认时，可在同一提交中提供本组修正字段。
    if correction_keys:

        # 使用当前组字段合同校验修正后的值。
        list_errors = validate_group_answers(  # 当前组修正值的验证诊断。
            {key: payload[key] for key in correction_keys},  # 仅提交本组修正字段。
            correction_keys,  # 指定需要逐项验证的字段顺序。
        )
