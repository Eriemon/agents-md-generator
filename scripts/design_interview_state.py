...
def autogenerated_takeover_answers(project: Path, state: dict[str, Any]) -> dict[str, Any]:
    ...
    if kind == "skill":
        synthesized = {
            ...
            "validation_gates": f"quick_validate.py {local_primary.rstrip('/')}; python {local_primary}scripts/audit_skill.py {local_primary.rstrip('/')}; python {local_primary}scripts/verify_agents.py . --installed-skill-dir {local_primary.rstrip('/')}; python {local_primary}scripts/manage_docs.py verify .; python {local_primary}scripts/evaluate_skill.py {local_primary.rstrip('/')} .",
    ...
    synthesized.update(
        {
            ...
            "feature_directory_rules": f"Place main project content under `{local_primary}`, keep tests under `tests/`, release artifacts under `dist/`, governance records under `docs/`, and keep detailed maintainability, long-task automation, and tool-script rules in the local JSON governance config instead of AGENTS.md text.",
