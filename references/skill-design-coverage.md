...
- Coding Behavior Baseline language skill routing that points to `coding_behavior.language_skill_routing` in `.agents/global-rule-overrides.json`, routes Python code generation/modification/commenting/normalization to `readable-python-generator`, routes bat/cmd, shell/bash, PowerShell, and Tcl script work to `readable-script-generator`, prevents Python targets from being routed to the script skill, and keeps detailed examples in `references/coding-behavior-language-routing.md`.
...
- Require the rendered root `AGENTS.md` to keep Coding Behavior Baseline language skill routing; verification must reject managed roots that remove the routing text, lose `coding_behavior.language_skill_routing`, weaken Python `readable-python-generator` routing, weaken script `readable-script-generator` routing, remove the Python-target guard, remove the script-wrapper guard, or weaken the formatting separation rule.
...
- The generated root includes Coding Behavior Baseline language skill routing, points to `coding_behavior.language_skill_routing`, routes Python work to `readable-python-generator`, routes script work to `readable-script-generator`, and verification fails if the routing is missing or weakened.
