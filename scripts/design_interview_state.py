...
def confirmed_keys_for_state(state: dict[str, Any], include_current: bool = False) -> list[str]:
    ...
    if include_current and state.get("status") in {"awaiting_group_confirmation", "awaiting_extra_requirements", "awaiting_final_alignment", "awaiting_design_review", "completed", "completed_read_only"}:
...
def remaining_groups_for_state(state: dict[str, Any]) -> list[list[str]]:
    groups = state.get("groups", [])
    index = int(state.get("current_group_index", 0))
    if state.get("status") in {"awaiting_extra_requirements", "awaiting_final_alignment", "awaiting_design_review", "awaiting_review_rework", "completed", "completed_read_only"}:
        return []
...
def interactive_payload(
    ...
    if status == "collecting_group":
        ...
        confirmation_question = "只读设计访谈已完成。保留 answers_snapshot 和 profile_preview 供解释、规划或人工复核使用；只有显式执行写入升格后才会触发子智能体审查。"
        next_action = "stay_read_only_or_enter_write_review"
    ...
    payload: dict[str, Any] = {
        ...
        "review_policy": review_policy_for_state(state, status),
    ...
    if status == "awaiting_remote_install_confirmation":
        ...
        payload["decision_request"] = decision_request(
            ...
            options=[
                ...
                {"label": "申请写入审查", "value": "enter_write_review", "description": "显式切换到写入意图并生成 design_review_request。", "recommended": False},
            ],
...
def complete_read_only(project: Path, state: dict[str, Any], final_answers: dict[str, Any], profile: dict[str, Any]) -> dict[str, Any]:
    ...
    state["status"] = "completed_read_only"
    state["profile_preview"] = profile
    state.pop("design_review_request", None)
...
def finalize_alignment(project: Path, state: dict[str, Any], payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if payload[ALIGNMENT_KEY]:
        ...
        if errors:
            ...
            raise SystemExit(1)
        if normalize_intent(state.get("intent")) == "read_only":
            return complete_read_only(project, state, final_answers, profile)
...
def enter_write_review(project: Path, state: dict[str, Any]) -> dict[str, Any]:
    ...
    if profile is None:
        ...
        if errors:
            emit_json(interactive_payload(project, state, errors=errors))
            raise SystemExit(1)
    return enter_design_review(project, state, answers, profile)
