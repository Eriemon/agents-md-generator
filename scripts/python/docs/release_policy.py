...
def governed_allowed_paths(profile: dict[str, Any], skill_dir: Path, project: Path) -> list[str]:
    ...
    if isinstance(list_configured, list) and list_configured:
        ...
        # 知识图谱门禁维护的根忽略文件属于发布治理自身生成的必要变更。
        list_allowed = [  # 规整后的发布准备允许路径
            str(item).replace("\\", "/").strip().strip("/")  # POSIX 相对路径
            for item in list_configured  # 原始白名单条目
            if str(item).strip()  # 过滤空路径
        ]

        # 知识图谱门禁可能在发布准备前补齐根忽略规则。
        if ".gitignore" not in list_allowed:

            # 根忽略文件纳入发布准备允许变更范围。
            list_allowed.append(".gitignore")
    ...
    return [str_rel_skill, "tests", "docs", ".agents", "AGENTS.md", ".gitignore", "dist"]
...
def is_excluded_dist_artifact(relative_path: str, excluded: set[str]) -> bool:
    ...
    # 历史快照路径先规整为 POSIX 形式再匹配排除前缀。
    str_normalized = relative_path.replace("\\", "/")  # 快照排除匹配使用的规整路径
