...
6. Before changing local or remote deployment folder structure, follow `docs/dir_manager/` and run `manage_dirs.py review` when directory governance is configured.
...
8. For remote server deployment tasks, do not sync local skill-development content to servers; deploy only explicit runtime/deployment artifacts unless the user explicitly overrides.
