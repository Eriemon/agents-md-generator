...
# 知识图谱共享模块提供画像合同的单一事实源。
from codebase_memory_mcp import codebase_memory_contract
...
def missing_answers(answers: dict[str, Any], kind: str) -> list[str]:
    ...
    for item in questions_for(kind):
        ...
        # 知识图谱选择必须是显式布尔值，不能把缺失或字符串当成禁用。
        if str_key == USE_CODEBASE_MEMORY_MCP_KEY:

            # 只有真实布尔值才能形成稳定启用或禁用合同。
            if not isinstance(answers.get(str_key), bool):

                # 类型错误与字段缺失统一登记为必答项未满足。
...
def directory_layout_policy(kind: str, name: str) -> dict[str, Any]:
    ...
    return {
        ...
        "tests_layout": {
            "required_root": "tests",
            "feature_depth": 1,
            "root_python_exemptions": ["__init__.py"],
            "feature_pattern": r"^[a-z]+(?:_[a-z]+)*$",
            "forbid_nested_tests": True,
        },
    }
