...
def memory_contract(project: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "storage_backend": str(
            dict_contract.get(
                "storage_backend",
                dict_profile.get("memory_storage_backend", "sqlite-plus-jsonl"),
            )
        ).strip()
        or "sqlite-plus-jsonl",
        "database": str(dict_contract.get("database", f"{str_folder}/memory.sqlite3")).strip()
        or f"{str_folder}/memory.sqlite3",
        "events": str(dict_contract.get("events", f"{str_folder}/events.jsonl")).strip()
        or f"{str_folder}/events.jsonl",
        "summaries": str(dict_contract.get("summaries", f"{str_folder}/summaries.md")).strip()
        or f"{str_folder}/summaries.md",
        "guide": str(dict_contract.get("guide", f"{str_folder}/MEMORY.md")).strip()
        or f"{str_folder}/MEMORY.md",
        "bootstrap_state": str(
            dict_contract.get("bootstrap_state", f"{str_folder}/bootstrap-state.json")
        ).strip()
        or f"{str_folder}/bootstrap-state.json",
        "capture_scope": str(
            dict_contract.get(
                "capture_scope",
                dict_profile.get("memory_capture_scope", DEFAULT_CAPTURE_SCOPE),
            )
        ).strip()
        or DEFAULT_CAPTURE_SCOPE,
        "read_policy": str(
            dict_contract.get(
                "read_policy",
                dict_profile.get("memory_read_policy", DEFAULT_READ_POLICY),
            )
        ).strip()
        or DEFAULT_READ_POLICY,
        "sensitivity_policy": str(
            dict_contract.get(
                "sensitivity_policy",
                dict_profile.get("memory_sensitivity_policy", DEFAULT_SENSITIVITY_POLICY),
            )
        ).strip() or DEFAULT_SENSITIVITY_POLICY,
        "compress_after_events": int(dict_contract.get("compress_after_events", 20) or 20),
    }
...
def init_memory(
    ...
    *,
    ...
    # bootstrap state 由会话 bootstrap 流程管理，不属于基础初始化必需文件。
    list_missing_paths = [  # 当前缺失的基础 memory 文件
        rel(project, path)  # 缺失设施的项目相对路径
        for key, path in dict_paths.items()  # 全部受治理设施路径
        if key != "bootstrap_state"  # bootstrap 文件由独立流程维护
        and (  # 目录与普通文件分别检查实际存在类型
            (key == "folder" and not path.is_dir())  # memory 根必须是目录
            or (key != "folder" and not path.is_file())  # 其他设施必须是普通文件
        )
    ]
    ...
    # 创建清单只记录本次真正新增的文件，供审计和测试使用。
    list_created: list[str] = []  # 本次新建的项目相对路径

    # 基础目录存在后才能创建事件流、摘要、指南和数据库。
    dict_paths["folder"].mkdir(parents=True, exist_ok=True)

    # 文本设施按用途选择初始内容，已有文件保持不变。
    for key in ("events", "summaries", "guide"):

        # 路径来自受治理契约，不在循环中自行拼接。
        path_target = dict_paths[key]  # 当前待检查的 memory 文本文件

        # 初始化必须是幂等操作，只处理缺失文件。
        if not path_target.exists():

            # JSONL 事件流从空文件开始，避免伪造初始化事件。
            if key == "events":

                # 空文本是合法的零事件 JSONL 状态。
                str_content = ""  # 当前文件的初始化正文

            # 人工摘要使用明确占位文本表示尚无持久化条目。
            elif key == "summaries":
            ...
            # guide 分支依据最终契约生成治理说明。
            else:

                # 指南内容必须反映最终启用后的契约字段。
                str_content = memory_guide(dict_contract)  # memory 治理指南正文

            # 所有治理文本使用 UTF-8，保持跨平台读取一致。
            path_target.write_text(str_content, encoding="utf-8")

            # 审计结果不暴露本机绝对路径。
            list_created.append(rel(project, path_target))

    # SQLite 文件由连接初始化创建，先登记预期新增路径。
    if not dict_paths["database"].exists():

        # 仅在连接前不存在时加入创建清单。
        list_created.append(rel(project, dict_paths["database"]))

    # 数据库打开和 schema 初始化可能因损坏文件失败，需要转换为治理结果。
    try:

        # 进入上下文并在退出时回收资源。
        with closing(connect_memory_db(dict_paths["database"])):

            # 空上下文主体表示这里只验证连接和初始化副作用。
            pass

    # SQLite 错误属于可报告的项目状态，不让 CLI 直接抛出堆栈。
    except sqlite3.DatabaseError as exc:

        # 保留已经创建的文本清单，并明确标注数据库失败位置。
        return {
            "project": str(project),
            "enabled": dict_contract["enabled"],
...
def write_memory(project: Path, input_path: str | Path) -> dict[str, Any]:
    ...
    # 调用方可保留历史创建时间，缺失时使用当前秒级时间。
    str_created_at = str(dict_data.get("created_at") or now_iso())  # 条目首次创建时间

    # 未显式更新的条目以创建时间作为首次更新时间。
    str_updated_at = str(dict_data.get("updated_at") or str_created_at)  # 条目最近更新时间

    # 人工列表字段优先于历史 tags_json 兼容字段。
    list_tags = normalize_tags(dict_data.get("tags") or dict_data.get("tags_json"))  # 归一化标签

    # 主表条目在安全检查前完成类型和缺省值归一化。
    dict_item = {  # 待校验并写入数据库的 memory 条目
        "id": "",  # 稳定标识稍后基于归一化字段生成
        "kind": str(dict_data.get("kind") or "note").strip() or "note",  # 主表类别字段
        "title": str(dict_data.get("title") or "Untitled memory").strip()  # 原始标题归一化
        or "Untitled memory",  # 人工可读标题
        "summary": str(dict_data.get("summary") or "").strip(),  # 必填摘要正文
        "source_path": str(dict_data.get("source_path") or "").strip(),  # 仓库相对来源路径
        "source_hash": str(dict_data.get("source_hash") or "").strip(),  # 主表来源哈希字段
        "source_ref": str(dict_data.get("source_ref") or "").strip(),  # 主表来源引用字段
        "source_timestamp": str(dict_data.get("source_timestamp") or "").strip(),  # 主表来源事件时间
        "sequence": int(dict_data.get("sequence") or 0),  # 来源事件顺序
        "tags_json": json.dumps(list_tags, ensure_ascii=False, sort_keys=True),  # 持久化标签 JSON
        "created_at": str_created_at,  # 主表首次创建时间
        "updated_at": str_updated_at,  # 主表最近更新时间
        "sensitivity": str(dict_data.get("sensitivity") or "normal").strip()  # 原始敏感级别
        or "normal",  # 敏感级别
    }

    # 显式 ID 优先，否则使用规范化内容生成确定性标识。
    dict_item["id"] = memory_item_id(  # 主表与搜索索引共享的条目标识
        {**dict_data, **dict_item}, str_created_at  # 原始 ID 与规范化字段共同参与
    )  # 当前条目的稳定数据库主键

    # 空摘要没有可复用事实，禁止写入占位记录。
    if not dict_item["summary"]:

        # 在打开数据库前返回验证错误，确保没有副作用。
        return {
            "project": str(project),
            "written": False,
            "errors": ["memory summary must not be empty"],
        }

    # 来源路径必须保持项目相对且不能越过仓库根。
    list_path_errors = source_path_errors(project, dict_item["source_path"])  # 来源路径错误
    ...
    str_sensitive_surface = " ".join(  # 汇总后执行一次安全扫描的输入文本
        str(dict_item.get(key, ""))  # 当前安全扫描字段值
        for key in ["title", "summary", "source_path"]  # 安全扫描字段白名单
    )  # 需要执行敏感信息检查的文本面

    # 安全检查结果在任何持久化操作之前计算。
    list_unsafe_errors = unsafe_summary_text(str_sensitive_surface)  # 敏感信息阻断项

    # 任一敏感信息命中都阻断主表和审计流写入。
    if list_unsafe_errors:

        # 诊断由统一安全检查器生成，保持各 memory 入口一致。
