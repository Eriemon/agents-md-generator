...
| Task rating gate contract | `task_rating_gate_contract` proves the global baseline prefers reuse first, does not ask difficulty/scale for every task, asks when complex or unclear work needs rating confirmation, preserves explicit/contextual hell or nightmare ratings without re-asking, and does not misread a difficulty-order list as a user rating |
