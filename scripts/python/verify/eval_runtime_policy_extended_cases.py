...
def case_plan_mode_language_lock_contract(
    ...
    # 通用语言锁文本用于证明原有自然语言约束仍然保留。
    str_generic_language_lock = (  # 根规则通用默认语言片段
        "All natural-language responses must use the configured default language (`中文`)"  # 通用语言锁原文
    )

    # 专项语言锁文本用于证明 proposed_plan 内容也受相同语言约束。
    str_plan_language_lock = (  # 根规则 Plan Mode 默认语言片段
        "In Plan Mode, any content inside `<proposed_plan>` must use the configured "  # 专项规则前半句
        "default language (`中文`)"  # 专项规则默认语言要求
    )

    # 正向检查覆盖通用规则、专项规则、参考文档和扫描实现。
    dict_with_checks = {  # Plan Mode 默认语言合同检查
        "repo_agents_mentions_generic_language_lock": str_generic_language_lock  # 根规则是否保留通用语言锁
        in dict_texts["agents"],  # 当前仓库根规则
        "repo_agents_mentions_plan_mode_lock": str_plan_language_lock  # 根规则是否含专项语言锁
        in dict_texts["agents"],  # proposed_plan 规则的仓库根文本
        ...
        "script_guide_mentions_plan_mode_lock": "Plan Mode" in dict_texts["script_guide"]  # 脚本指南是否提及 Plan Mode
        and "<proposed_plan>" in dict_texts["script_guide"],  # 脚本指南是否绑定计划标签
        "review_checklist_mentions_plan_mode_lock": "Plan Mode" in dict_texts["review_checklist"]  # 审查清单是否提及 Plan Mode
        and "<proposed_plan>" in dict_texts["review_checklist"],  # 审查清单是否检查计划标签
...
def case_codex_token_usage_review_contract(
    ...
    # agent 元数据向模型重复显式触发边界，避免默认读取私人会话日志。
    str_openai_text = (SKILL_DIR / "agents" / "openai.yaml").read_text(encoding="utf-8")  # agent 触发描述

    # 脚本指南规定只有可解析且存在的 Codex sessions 树允许执行。
    ...
    dict_with_checks = {  # 显式触发、目录边界和数值汇总三层合同检查
        ...
        "skill_mentions_explicit_trigger": "如果用户明确要求进行 Codex Token 用量统计"  # 中文技能规则是否要求显式触发
        in str_skill_text,  # 中文技能正文是否声明用户显式触发边界
        "openai_mentions_explicit_trigger": "explicitly asks for Codex token usage statistics"  # agent 元数据是否要求显式触发
        in str_openai_text,  # agent 元数据是否重复显式触发约束
        "script_guide_mentions_guard": "仅在当前环境可解析到 `$CODEX_HOME/sessions` 或 `~/.codex/sessions` 且目录存在时执行"  # 指南是否声明目录前置条件
        in str_script_guide,  # 安全执行前置条件所在的指南正文
