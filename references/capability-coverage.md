...
| Version and freshness gates | `manage_docs.py verify`, `version_alignment_gate`, `sync-root-agents --mark-verified`, and `check_freshness.py` keep source `VERSION`, root AGENTS metadata/control-profile version, development/changelog/git-manager records, and Last verified freshness aligned |
...
| Confidence aggregation | `run_confidence_gate.py --review-base <sha>` combines quick validation, audit, docs/AGENTS verification, evaluate, skill evals, work-folder governance, freshness, automated review, release gates, install skip, and optional external-skill smoke evidence |
