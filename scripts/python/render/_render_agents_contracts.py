...
def conversation_completion_contract(profile: dict | None) -> str:
    ...
    return "\n".join([
        ...
        (
            ...
            f"(`{default_language}`) unless the user explicitly switches languages. In "
            f"Plan Mode, any content inside `<proposed_plan>` must use the configured "
            f"default language (`{default_language}`) unless the user explicitly switches "
            f"languages. Keep the `<proposed_plan>` tags unchanged; code, commands, logs, "
            f"raw error text, and proper nouns may remain in their original form."
        ),
        "- If work cannot be completed, report blockers, completed files, "
        "unverified assumptions, and exact next steps.",
...
def task_specific_gates(profile: dict | None, project: Path) -> str:
    ...
    list_contract_groups = [
        ...
        (
            ...
            "docs",  # 文档规则 ID 前缀
            ("Before new work", "Start work", "Every completed", "Directory changes"),  # 文档阻断片段
            20,  # 文档治理优先级
        ),
        (
...
def coding_behavior_baseline(project: Path, profile: dict | None, facts: dict | None = None) -> str:
    ...
    # 编码行为完全从规则覆盖文件读取。
    policy = load_global_rule_overrides(project, profile)["data"].get(  # 编码行为策略
