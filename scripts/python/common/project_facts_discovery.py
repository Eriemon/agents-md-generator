...
# 配置文件发现器集中维护受支持的根级生态清单。
def project_config_files(root: Path) -> list[str]:
    """返回项目中实际存在的生态配置与工作区设置。

    参数：root 为项目根目录。
    返回：保持候选优先级的项目相对配置路径列表。
    """
...
# 自动化事实发现器汇总 CI 提供方和 AI 协作配置。
def automation_facts(root: Path) -> tuple[list[str], list[str]]:
    """返回项目中实际存在的 CI 与 AI 配置事实。

    参数：root 为项目根目录。
    返回：CI 提供方列表与 AI 配置相对路径列表。
    """
    ...
    # 调用方按固定元组位置合并两类自动化事实。
    return list_ci, list_ai_configs
...
def project_stack_facts(root: Path) -> dict[str, Any]:
    ...
    # 配置发现器保留既有根文件与工作区设置顺序。
    list_config_files = project_config_files(root)  # 项目配置文件列表。
    ...
    # 自动化发现器保持 CI 与 AI 配置的原始优先级。
    tuple_automation = automation_facts(root)  # CI 与 AI 配置事实元组。

    # 首项保留持续集成提供方顺序。
    list_ci = tuple_automation[0]  # 自动化事实中的 CI 顺序。

    # 第二项保留 AI 协作配置候选顺序。
    list_ai_configs = tuple_automation[1]  # AI 协作配置列表。
...
# 源码后缀扫描补充缺少包管理配置的小型项目语言事实。
def source_languages(root: Path, list_configured_languages: list[str]) -> list[str]:
    """按首次发现顺序补充源码后缀对应的语言类别。

    参数:
        root: 待扫描项目根目录。
        list_configured_languages: 技术栈配置已发现的语言列表。

    返回:
        保留配置优先级并补充源码证据的语言列表。
    """

    # 复制输入列表，避免项目检查修改技术栈发现器返回值。
    list_languages = list(list_configured_languages)  # 可扩展语言优先级列表

    # 源码后缀映射覆盖受治理的编程和脚本语言。
    dict_source_suffix_languages = {
        ".py": "python",  # Python 模块触发可读 Python 规则路由
        ".c": "c",  # C 实现文件提供原生语言项目证据
        ".cc": "cpp",  # 短后缀实现归并到 C++ 技术栈
        ".cpp": "cpp",  # 常规后缀确认 C++ 源码存在
        ".cxx": "cpp",  # 扩展后缀补充 C++ 识别兼容性
        ".h": "c",  # C 接口头文件保留 C 语言归属
        ".hpp": "cpp",  # 模板头文件标记 C++ 接口类型
        ".v": "verilog",  # Verilog 设计源启用 RTL 专属路由
        ".sv": "systemverilog",  # SystemVerilog 单元保留独立语言标签
        ".bat": "script",  # 批处理入口启用 Windows 脚本审查
        ".cmd": "script",  # 命令文件纳入批处理脚本约束
        ".sh": "script",  # Shell 自动化启用脚本技能门禁
        ".ps1": "script",  # PowerShell 入口纳入脚本命名审查
        ".psm1": "script",  # PowerShell 模块纳入注释质量审查
        ".tcl": "script",  # Tcl 工具流程采用脚本治理类别
    }
    ...
    for path_source in root.rglob("*"):
        ...
        if not path_source.is_file() or any(
            str_part in SKIP_DIRS
            for str_part in path_source.relative_to(root).parts
        ):
        ...
        # 将规范化后缀映射为语言类别。
        str_language = dict_source_suffix_languages.get(path_source.suffix.lower())  # 当前文件语言
...
# 根规则载荷构造器保持 inspect 输出的字段名称与顺序。
def root_agents_payload(
    root: Path,
    tuple_agents: tuple[Any, ...],
    tuple_structure: tuple[bool, list[str]],
) -> dict[str, Any]:
    """把根规则、会话、全局基线和目录状态转换为公开字段。

    参数：root 为项目根，tuple_agents 为根规则事实，tuple_structure 为目录事实。
    返回：保持 inspect_project 既有顺序的根治理字段。
    """
    ...
    # 触发原因同时服务 trigger 与 rebuild 兼容字段。
    ...
    return {
        ...
        "current_skill_version": tuple_agents[3],
        "installed_skill_version": tuple_agents[2],
        ...
        "root_agents_md_repair_command": tuple_agents[5],
        ...
        "session_history_bootstrap_required": tuple_agents[7],
        ...
        "structure_fix_confirmation_required": tuple_structure[0],
        "structure_fix_default": "yes",
        "structure_fix_reasons": tuple_structure[1],
    }

# 源码治理载荷构造器集中扫描实现边界、覆盖配置与脚本布局。
def project_governance_payload(root: Path, dict_profile: dict[str, Any]) -> dict[str, Any]:
    """返回项目实现约束与源码治理公开字段。

    参数：root 为项目根，dict_profile 为项目控制配置。
    返回：保持 inspect_project 既有顺序的源码治理字段。
    """
    ...
    # 扫描源码尺寸、测试边界和注释策略。
    dict_source_governance = source_governance_report(root, dict_profile)  # 源码治理报告。

    # 检查工具脚本布局和三元组合同。
    dict_script_layout = script_layout_facts(root, dict_profile)  # 脚本布局报告。

    # 加载并验证项目全局规则覆盖。
    dict_overrides = load_global_rule_overrides(root, dict_profile)  # 规则覆盖报告。
...
# 技术栈载荷构造器补充源码语言并输出文件系统摘要。
def project_stack_payload(root: Path, dict_stack: dict[str, Any]) -> dict[str, Any]:
    """返回语言、框架、配置与目录文件摘要字段。

    参数：root 为项目根，dict_stack 为配置驱动技术栈事实。
    返回：保持 inspect_project 既有顺序的技术栈字段。
    """

    # 配置语言优先，源码后缀只补充缺失类别。
    list_languages = source_languages(root, list(dict_stack["languages"]))  # 完整语言优先级列表。

    # 技术字段顺序保持从主语言到文件摘要的既有协议。
    return {
...
def inspect_project(root: Path) -> dict[str, Any]:
    ...
    dict_profile = read_json(root / ".agents" / "agents-control.json")  # 当前项目控制配置。
    ...
    # 分区载荷按既有协议顺序合并为一个公开事实映射。
    return {
        **root_agents_payload(root, tuple_agents, tuple_structure),
        **project_governance_payload(root, dict_profile),
        **project_stack_payload(root, dict_stack),
    }
