...
`manage_dirs.py structure-gate` is the directory equivalent. It checks the enforced `primary_project_root`, allowed top-level roots, and current structure drift without mutating the repository. If blocked, the caller must ask whether to normalize the structure first; the recommended default is yes. `apply-structure-fix` performs only the conservative, explicit structure moves that were confirmed, and normal AGENTS generation should continue only after that.
...
`evolve` writes automatic evolution templates after accepted experience passes quality checks and AI-authored `evolution_summary` content exists. Every 10 completed handoffs, payload application triggers the same flow automatically. The summary for `1-workflow.md`, `2-scripts.md`, `3-plan.md`, and `4-design-ui.md` must synthesize the current plus latest historical versions into sections for evidence sources, applicable scenario, distilled workflow, key decisions, common problems, non-reusable content, and application checklist. Scripts must not copy `## Iterated Lessons` directly into reusable templates. If the summary is missing or fails quality checks, `evolve` writes `.agents/evolution-update-request.json` and refuses to write low-quality templates. When an old `evolution-index.json` points to obsolete cross-family output, the obsolete files are archived under `assets/templates/evolution/history_evolution/YYYYMMDD-HHMMSS/` before the new classified templates are written.
