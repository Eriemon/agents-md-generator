...
| Design profile | grouped design interview completed with `collect_design_profile.py --start/--resume/--answer-file`, `extra_requirements` recorded, final alignment confirmed, subagent `design_review` approved with matching hashes, then `python skills/agents-md-generator/scripts/collect_design_profile.py <project> --answers answers.json --write` completed when strong control is required |
...
| Automated review governance | `python skills/agents-md-generator/scripts/review_governance.py <project> --base <sha> --head HEAD --skill-dir skills/<skill-name> --mode all` passes before release or merge when governance-sensitive files changed |
...
- Only the `agents-md-generator` owner repository should evolve accepted experience into indexed templates under `assets/templates/evolution/`; ordinary user work folders must keep local handoff/experience governance without generating local evolution templates.
- Ordinary user work folders may still produce tenth-handoff reusable template output, but the active sink must be `installed-sink` or `export-pending`; they must not keep active local `assets/templates/evolution/` trees after cleanup.
- Tenth-handoff requests should expose `evolution_sink`, and `manage_docs.py import-evolution` should be reviewed whenever `export-pending` bundles are expected to move into the installed skill library.
- For the owner repository, every tenth handoff also requires an extra evolution review gate: `requires_extra_evolution_review=true`, `review_scope=evolution-only`, `review_blocking=true`, and a populated `review_contract` plus `release_alignment_evidence` must appear in `.agents/experience-update-request.json`.
...
- Owner-repository tenth-handoff payloads must include `evolution_review` with `verdict`, `approved_target`, `original_target`, `evidence_read`, `session_reread_performed`, `session_reread_reason`, and `full_explanation`; missing review state must block evolution and write `.agents/evolution-review-request.json`.
- Rejecting or overriding review outcomes must be auditable: `reject` blocks immediately, `approve_with_override` blocks until the payload target is resubmitted to match the approved target, and both paths must produce a review request that names conflicts and required explanation sections.
- The extra evolution review may cite Codex sessions only from exact-cwd matches. If `session_ids` or `session_paths` include other workspaces, the review is invalid and template writing must stay blocked.
- Evolution summaries must pass both path-level matching and content-level type isolation: a skill template must not carry engineering execution stages, and an engineering template must not carry AGENTS/control-profile/docs-governance workflow stages.
...
- If a non-owner work folder already contains mistaken local `assets/templates/evolution/` output, governance cleanup should archive it under `docs/dir_manager/history_dir_manager/<timestamp>/legacy-evolution/` before removing the active copy.
...
- Replacement installs should absorb prior reusable template knowledge by section-level merge when the installed and release templates are parseable; fallback `.installed-template-conflict` files are only acceptable for non-parseable inputs and must be reported explicitly.
